...
class ColorBT(gdb.Command):
    ...
    def printFrame (self, count, frame):
        if not frame.is_valid():
            return
        print("#%2d %s0x%x%s in %s%s%s" %
              (count, PC, frame.pc(), ENDC,
              FUNCTION, frame.name(), ENDC))
        sal = frame.find_sal()
        if sal.is_valid():
            if sal.symtab != None:
                print(" at %s%s%s\b%s:%s\b%s%d%s" %
                      (FILE, frame.find_sal().symtab.filename, ENDC,
                      FAIL, ENDC,
                      LINE, frame.find_sal().line, ENDC))
            else:
                print(sal)
        return
    ...
    def print_backtrace_frame(self, match_obj):
        ...
        print("#%s %s%s%s in %s%s%s(%s%s%s)%s %s%s%s" %
              (_frame_num, PC, _addr, ENDC,
              FUNCTION, _function, ENDC,
              FILE, _args, ENDC,
              _atfrom, FILE, _file, ENDC))
        print("")
        return
    ...
    def print_backtrace_frame_short(self, match_obj):
        ...
        print("#%s\n%s%s%s(%s%s%s)\n%s %s%s%s\n" %
              (_frame_num, FUNCTION, _function, ENDC,
              FILE, _args, ENDC,
              _atfrom, FILE, _file, ENDC))
        return
...
class ColorBTObj(gdb.Command):
    ...
    def printFrame (self, count, frame):
        ...
        if sal.is_valid():
            if sal.symtab != None:
                print(" at %s%s%s" %
                      (FILE, frame.find_sal().symtab.objfile.filename, ENDC))
            else:
                print(sal,)
        print("")
        return
